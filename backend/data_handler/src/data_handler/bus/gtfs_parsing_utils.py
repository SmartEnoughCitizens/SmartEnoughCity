...
def parse_gtfs_time(time_str: str) -> time:
    ...
    if len(parts) < 2:
        raise ValueError(f"Invalid time format: {time_str}. Expected HH:MM:SS format.")
    
    return time(
        int(parts[0]) % 24, # TODO: examine why time entries with hours > 24 are present in the GTFS stop_times data
        int(parts[1]),
