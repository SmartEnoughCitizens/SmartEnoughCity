...
def process_bus_static_data(gtfs_dir: Path) -> None:
    ...
    csv_files = {
        "agency.txt": (["agency_id", "agency_name", "agency_url", "agency_timezone"], parse_agency_row),
        "calendar.txt": (["service_id", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday", "start_date", "end_date"], parse_calendar_row),
        "routes.txt": (["route_id", "agency_id", "route_short_name", "route_long_name"], parse_route_row),
        "shapes.txt": (["shape_id", "shape_pt_lat", "shape_pt_lon", "shape_pt_sequence", "shape_dist_traveled"], parse_shape_row),
        "stop_times.txt": (["trip_id", "arrival_time", "departure_time", "stop_id", "stop_sequence"], parse_stop_time_row),
        "stops.txt": (["stop_id", "stop_code", "stop_name", "stop_lat", "stop_lon"], parse_stop_row),
        "trips.txt": (["route_id", "service_id", "trip_id", "trip_headsign", "trip_short_name", "direction_id", "shape_id"], parse_trip_row),
    }
    ...
    try:
        ...
        for filename, (required_headers, transform_row) in csv_files.items():
            ...
            file_path = gtfs_dir / filename
            rows = [transform_row(row) for row in read_csv_file(file_path, required_headers)]
        ...
        session.rollback()
        logger.error(f"Error processing static bus data: {e}")
        raise
