...
def fetch_all_stations(station_type: str = "A") -> list[dict]:
    ...
    try:
        ...
        logger.error("Failed to parse stations XML: %s", e)
        return []
    ...
    try:
        ...
        for s in stations_data:
            station = IrishRailStation(
                ...
                station_alias=s.get("StationAlias", "").strip() or None,
        ...
        session.rollback()
        logger.exception("Error inserting stations: %s", e)
...
def _update_station_types() -> None:
    ...
    try:
        ...
        session.rollback()
        logger.exception("Error updating station types: %s", e)
...
def fetch_current_trains(train_type: str = "A") -> list[dict]:
    ...
    try:
        ...
        logger.error("Failed to fetch current trains: %s", e)
        return []
    ...
    try:
        ...
        logger.error("Failed to parse trains XML: %s", e)
        return []
    ...
    try:
        ...
        for t in trains_data:
            train = IrishRailCurrentTrain(
                train_code=t.get("TrainCode", "").strip(),
                train_date=t.get("TrainDate", "").strip(),
                train_status=t.get("TrainStatus", "").strip(),
                train_type=t.get("TrainType", "").strip() or None,
                direction=t.get("Direction", "").strip() or None,
                lat=_safe_float(t.get("TrainLatitude")),
                lon=_safe_float(t.get("TrainLongitude")),
                public_message=t.get("PublicMessage", "").strip() or None,
        ...
        session.rollback()
        logger.exception("Error inserting current trains: %s", e)
...
def fetch_station_data(station_code: str, num_mins: int = 90) -> list[dict]:
    ...
    try:
        ...
        logger.error("Failed to fetch station data for %s: %s", station_code, e)
        return []
    ...
    try:
        ...
        logger.error("Failed to parse station data XML: %s", e)
        return []
...
def irish_rail_station_data_to_db() -> None:
    ...
    try:
        ...
        for station_code in station_codes:
            ...
            for a in arrivals:
                data = IrishRailStationData(
                    ...
                    train_code=a.get("Traincode", "").strip(),
                    train_date=a.get("Traindate", "").strip(),
                    train_type=a.get("Traintype", "").strip() or None,
                    origin=a.get("Origin", "").strip(),
                    destination=a.get("Destination", "").strip(),
                    origin_time=a.get("Origintime", "").strip() or None,
                    destination_time=a.get("Destinationtime", "").strip() or None,
                    status=a.get("Status", "").strip() or None,
                    last_location=a.get("Lastlocation", "").strip() or None,
                    due_in=_safe_int(a.get("Duein")),
                    late=_safe_int(a.get("Late")),
                    exp_arrival=a.get("Exparrival", "").strip() or None,
                    exp_depart=a.get("Expdepart", "").strip() or None,
                    sch_arrival=a.get("Scharrival", "").strip() or None,
                    sch_depart=a.get("Schdepart", "").strip() or None,
                    direction=a.get("Direction", "").strip() or None,
                    location_type=a.get("Locationtype", "").strip() or None,
                    fetched_at=fetched_at,
                )
        ...
        session.rollback()
        logger.exception("Error inserting station data: %s", e)
...
def fetch_train_movements(train_code: str, train_date: str) -> list[dict]:
    ...
    try:
        ...
        logger.error("Failed to fetch train movements for %s: %s", train_code, e)
        return []
    ...
    try:
        ...
        logger.error("Failed to parse train movements XML: %s", e)
        return []
...
def irish_rail_train_movements_to_db() -> None:
    ...
    try:
        ...
        for train_code, train_date in trains:
            ...
            for m in movements:
                movement = IrishRailTrainMovement(
                    train_code=m.get("TrainCode", "").strip(),
                    train_date=m.get("TrainDate", "").strip(),
                    location_code=m.get("LocationCode", "").strip(),
                    location_full_name=m.get("LocationFullName", "").strip(),
                    location_order=int(m.get("LocationOrder", 0)),
                    location_type=m.get("LocationType", "").strip(),
                    train_origin=m.get("TrainOrigin", "").strip(),
                    train_destination=m.get("TrainDestination", "").strip(),
                    scheduled_arrival=m.get("ScheduledArrival", "").strip() or None,
                    scheduled_departure=m.get("ScheduledDeparture", "").strip() or None,
                    actual_arrival=m.get("Arrival", "").strip() or None,
                    actual_departure=m.get("Departure", "").strip() or None,
                    ...
                    stop_type=m.get("StopType", "").strip() or None,
                    fetched_at=fetched_at,
                )
        ...
        session.rollback()
        logger.exception("Error inserting train movements: %s", e)
