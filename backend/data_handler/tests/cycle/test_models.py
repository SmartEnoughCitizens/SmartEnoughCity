...
class TestDublinBikesStationSnapshotModel:
    ...
    def test_model_has_required_fields(self) -> None:
        ...
        required_fields = {
            'station_id', 'timestamp', 'last_reported',
    ...
    def test_can_create_snapshot_with_foreign_key(self, in_memory_db) -> None:
...
class TestDublinBikesStationHistoryModel:
    ...
    def test_unique_constraint_on_station_timestamp(self) -> None:
        ...
        has_unique = any(
            'station_id' in [col.name for col in c.columns] and
            'timestamp' in [col.name for col in c.columns]
            for c in unique_constraints
        )
        assert has_unique, "Missing unique constraint on (station_id, timestamp)"

    def test_cannot_insert_duplicate_station_timestamp(self, in_memory_db) -> None:
