...
@Repository
public interface TrainStationDataRepository
        extends JpaRepository<TrainStationData, Integer> {

    /**
     * Latest station-data record per (station_code, train_code) — one row per
     * currently-expected train at each station.
     */
    @Query("""
    ...
    /**
     * Fraction of records where lateMinutes > 0 (late arrival %).
