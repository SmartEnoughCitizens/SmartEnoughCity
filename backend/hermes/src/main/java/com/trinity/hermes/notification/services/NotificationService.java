...
public class NotificationService {
   ...
   */
  public Set<Notification> createNotification(User user, Object... payloads) {
    try {
      Map<String, Object> data = buildPayloadMap(payloads);
      byte[] qrCode = null;
      if (data.containsKey(QR_ID)
          && StringUtils.isNotBlank(Objects.toString(data.get(QR_ID), EMPTY_STRING))) {
  ...
  private Set<Notification> createFromTemplate(
      String recipient, Map<String, Object> data, byte[] qrCode) {
    try {
      String templateKey =
          Objects.toString(data.getOrDefault(TEMPLATE_KEY, EMPTY_STRING), EMPTY_STRING);
      ...
      if (Objects.isNull(template)) {
        log.error(
            "No template '{}' and no default template available. Returning empty set.",
            templateKey);
        return Set.of();
      }
