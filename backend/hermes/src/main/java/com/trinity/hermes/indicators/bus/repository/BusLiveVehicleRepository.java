...
public interface BusLiveVehicleRepository extends JpaRepository<BusLiveVehicle, Integer> {
    ...
    @Query(value = "SELECT DISTINCT ON (vehicle_id) *"
            + " FROM external_data.bus_live_vehicles"
            + " ORDER BY vehicle_id, timestamp DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(DISTINCT vehicle_id) FROM external_data.bus_live_vehicles"
            + " WHERE timestamp >= NOW() - INTERVAL '5 minutes'", nativeQuery = true)
    Long countActiveVehicles();

    @Query(value = "SELECT DISTINCT ON (blv.vehicle_id) blv.*"
            ...
            + " ORDER BY blv.vehicle_id, blv.timestamp DESC", nativeQuery = true)
