...
public class DisruptionController {
  ...
  /**
   * Main endpoint for disruption detection from Python data handler service. This is called when
   * the Python service detects a disruption in real-time data.
   *
   * <p>POST /api/v1/disruptions/detect
   *
  ...
  public ResponseEntity<DisruptionDetectionResponse> detectDisruption(
    ...
    log.info(
        "Received disruption detection request from Python service: {}",
    ...
    try {
      ...
      if (solution == null) {
        ...
        return ResponseEntity.ok(
            new DisruptionDetectionResponse(false, null, "Disruption ignored: Threshold not met"));
      }

      return ResponseEntity.status(HttpStatus.CREATED)
          .body(
              new DisruptionDetectionResponse(
                  true,
                  solution.getDisruptionId(),
  ...
  /** Get all active disruptions GET /api/v1/disruptions/active */
  @GetMapping("/active")
  ...
  /** Get disruptions by severity GET /api/v1/disruptions/severity/{severity} */
  @GetMapping("/severity/{severity}")
  public ResponseEntity<List<DisruptionResponse>> getDisruptionsBySeverity(
  ...
  /** Resolve a disruption (mark as no longer active) POST /api/v1/disruptions/{id}/resolve */
  ...
  /** Get incident logs for a disruption GET /api/v1/disruptions/{id}/logs */
  @GetMapping("/{id}/logs")
  ...
  public ResponseEntity<DisruptionResponse> getDisruptionById(@PathVariable Long id) {
    ...
    return disruption.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
  }

  @PostMapping
  public ResponseEntity<DisruptionResponse> createDisruption(
      @RequestBody CreateDisruptionRequest request) {
    DisruptionResponse created = disruptionFacade.createDisruption(request);
    return ResponseEntity.status(HttpStatus.CREATED).body(created);
  }

  @PutMapping("/{id}")
  public ResponseEntity<DisruptionResponse> updateDisruption(
      @PathVariable Long id, @RequestBody UpdateDisruptionRequest request) {
