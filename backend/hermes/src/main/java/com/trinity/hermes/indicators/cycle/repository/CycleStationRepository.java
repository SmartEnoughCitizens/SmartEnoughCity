...
public interface CycleStationRepository extends JpaRepository<CycleStation, String> {
  ...
  @Query(
      "SELECT AVG(CAST(c.numBikesAvailable AS double) / NULLIF(c.capacity, 0) * 100) FROM CycleStation c WHERE c.capacity > 0")
  Double findAverageOccupancyRate();

  @Query(
      "SELECT c FROM CycleStation c WHERE c.lat BETWEEN :minLat AND :maxLat AND c.lon BETWEEN :minLon AND :maxLon")
