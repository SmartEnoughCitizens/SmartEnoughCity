...
regional_monthly = (
    car_counts.groupby(["Final_Region", "Year", "Month"])["Sum_Volume"]
    .sum()
    .reset_index()
)

regional_car_counts = (
    regional_monthly.groupby("Final_Region")["Sum_Volume"]
...
households = households.merge(
    regional_car_counts, left_on="Region", right_on="Final_Region", how="left"
)

# Estimated number of EVs in each household cluster
households["Estimated_EVs"] = (
    households["Household_Proportion"] * households["Avg_Car_Count"] * EV_ADOPTION_RATE
)
