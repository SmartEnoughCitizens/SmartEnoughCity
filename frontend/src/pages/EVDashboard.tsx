 ...
 * Displayed as a sub-tab of CarDashboard.
 *
 * Map caching: MapContainer is always rendered so Leaflet is not re-initialised
...
const getDemandColor = (demand: number, max: number): string => {
  ...
  if (ratio > 0.66) return "#fca5a5";
  if (ratio > 0.33) return "#fdba74";
  return "#86efac";
};
  ...
  const handleResultClick = (result: SearchResult) => {
    ...
    if (result.type === "area") {
      ...
      if (areaFeature) {
        ...
        const avgLng =
          ring.reduce((sum: number, c: number[]) => sum + c[0], 0) /
          ring.length;
        const avgLat =
          ring.reduce((sum: number, c: number[]) => sum + c[1], 0) /
          ring.length;
