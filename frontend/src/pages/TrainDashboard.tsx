...
type PanelTab = "stations" | "trains" | "delays" | "recommendations";
...
export const TrainDashboard = () => {
  ...
  const switchTab = (tab: PanelTab) => {
    ...
    else if (tab === "delays" || tab === "recommendations") {
      setSelectedStationCode(null);
      setSelectedTrainCode(null);
    ...
    >
      ...
      {panelOpen && (
        ...
        >
          ...
          >
            {(["stations", "trains", "delays", "recommendations"] as PanelTab[]).map((tab) => {
          ...
          {/* Count caption */}
          {activeTab !== "delays" && activeTab !== "recommendations" && (
            <Box sx={{ px: 2, pb: 0.5 }}>
              <Typography
                variant="caption"
              ...
              {activeTab === "recommendations" && (
                <Box sx={{ p: 1.5 }}>
                  {recommendationsLoading ? (
                    ...
                    <Typography
                      sx={{ color: "#8b949e", fontSize: "0.8rem", textAlign: "center", py: 4 }}
                    >
                    ...
                    >
                      <Box component="thead">
                        <Box component="tr">
                          {["Train Name", "Status", "Current Count", "Predicted Count", "Recommendation", "Simulation"].map((h) => (
                            <Box
                              component="th"
                              key={h}
                              sx={{
                      ...
                      <Box component="tbody">
                        {trainRecommendations.map((rec) => {
                          let rows: Array<{ trainName: string; status: string; currentCount: number; predictedCount: number; recommendation: string }> = [];
                          try {
                            const parsed = JSON.parse(rec.recommendation);
                            if (Array.isArray(parsed)) {
                              rows = parsed.map((item: { "Train Name": string; Attributes: { status: string; Current_count: number; Predicted_count: number; Recommendation: string } }) => ({
                          ...
                          return rows.map((row, idx) => (
                            <Box
                              component="tr"
                              key={`${rec.id}-${idx}`}
                              sx={{ "&:hover td": { bgcolor: "rgba(255,255,255,0.03)" } }}
                            >
                              <Box component="td" sx={{ py: 0.75, px: 1, color: "#e6edf3", fontWeight: 600, borderBottom: "1px solid rgba(48,54,61,0.3)", whiteSpace: "nowrap" }}>
                                {row.trainName}
                              </Box>
                              <Box component="td" sx={{ py: 0.75, px: 1, borderBottom: "1px solid rgba(48,54,61,0.3)" }}>
                                <Chip
                                  size="small"
                                  label={row.status}
                                  sx={{
                                    fontSize: "0.62rem",
                                    height: 18,
                                    bgcolor: row.status === "over-utilised" ? "#da362320" : "#d2992220",
                                    color: row.status === "over-utilised" ? "#da3623" : "#d29922",
                                    border: `1px solid ${row.status === "over-utilised" ? "#da362340" : "#d2992240"}`,
                                  }}
                                />
                              </Box>
                              <Box component="td" sx={{ py: 0.75, px: 1, color: "#c9d1d9", borderBottom: "1px solid rgba(48,54,61,0.3)", whiteSpace: "nowrap" }}>
                                {row.currentCount.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              </Box>
                              <Box component="td" sx={{ py: 0.75, px: 1, color: "#c9d1d9", borderBottom: "1px solid rgba(48,54,61,0.3)", whiteSpace: "nowrap" }}>
                                {row.predictedCount.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                              </Box>
                              <Box component="td" sx={{ py: 0.75, px: 1, color: "#8b949e", borderBottom: "1px solid rgba(48,54,61,0.3)" }}>
                                {row.recommendation}
                              </Box>
                              <Box component="td" sx={{ py: 0.75, px: 1, borderBottom: "1px solid rgba(48,54,61,0.3)", textAlign: "center" }}>
                                {rec.simulation && (
                                  <IconButton
                                    size="small"
                                    onClick={() => setSimulationModal({ open: true, simulation: rec.simulation })}
                                    sx={{ color: "#d29922", p: 0.25, "&:hover": { color: "#f0b429" } }}
                                  >
      ...
      >
        <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1, borderBottom: "1px solid rgba(48,54,61,0.5)", pb: 1.5 }}>
          <LightbulbIcon sx={{ color: "#d29922", fontSize: 20 }} />
          <Typography fontWeight={700} sx={{ color: "#e6edf3", fontSize: "1rem" }}>
            Simulation Results
          </Typography>
        </DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          {simulationModal.simulation && (() => {
            let results: Array<{ headsign: string; new_utilisation_pct: number; passengers_absorbed: number; co2_saved_kg: number; sustainability_score: number; summary: string }> = [];
            try {
              const parsed = JSON.parse(simulationModal.simulation);
            ...
            return results.map((r, idx) => (
              ...
              >
                <Typography fontWeight={700} sx={{ color: "#e6edf3", fontSize: "0.875rem", mb: 1 }}>
                  {r.headsign}
                </Typography>
                <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 1 }}>
                  <Chip size="small" label={`${r.new_utilisation_pct.toFixed(1)}% utilisation`} sx={{ fontSize: "0.65rem", height: 18, bgcolor: r.new_utilisation_pct > 100 ? "#da362320" : "#2ea04320", color: r.new_utilisation_pct > 100 ? "#da3623" : "#2ea043", border: `1px solid ${r.new_utilisation_pct > 100 ? "#da362340" : "#2ea04340"}` }} />
                  <Chip size="small" label={`${r.passengers_absorbed.toFixed(0)} passengers absorbed`} sx={{ fontSize: "0.65rem", height: 18, bgcolor: "rgba(255,255,255,0.05)", color: "#8b949e" }} />
                  <Chip size="small" label={`${r.co2_saved_kg.toFixed(0)} kg CO₂ saved`} sx={{ fontSize: "0.65rem", height: 18, bgcolor: "#2ea04310", color: "#2ea043" }} />
                  <Chip size="small" label={`Sustainability: ${r.sustainability_score}`} sx={{ fontSize: "0.65rem", height: 18, bgcolor: "rgba(255,255,255,0.05)", color: "#8b949e" }} />
                </Box>
                <Typography sx={{ fontSize: "0.75rem", color: "#8b949e", lineHeight: 1.5 }}>
                  {r.summary}
                </Typography>
              </Box>
            ));
        ...
        <DialogActions sx={{ borderTop: "1px solid rgba(48,54,61,0.5)", px: 2, py: 1 }}>
          <Button
            size="small"
            onClick={() => setSimulationModal({ open: false, simulation: null })}
