...
function KpiCard({ icon, label, value, sub, color = "primary.main" }: KpiCardProps) {
    return (
        <Box
            sx={{
                display: "flex",
...
export const TrainDashboard = () => {
    ...
    const liveColor = (status?: string) => (isRunning(status) ? "#4caf50" : "#ff9800");
                    ...
                    {kpiData && (
                        <Box sx={{ px: 2, pb: 1.5, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1 }}>
                            ...
                            <KpiCard
                                ...
                                color={kpiData.onTimePct >= 80 ? "success.main" : "warning.main"}
                            />
                            <KpiCard
                                icon={<SpeedIcon fontSize="inherit" />}
                                label="Avg Delay"
                                value={`${kpiData.avgDelayMinutes.toFixed(1)} min`}
                                color={kpiData.avgDelayMinutes <= 2 ? "success.main" : "warning.main"}
                            />
                        </Box>
                    )}
